...
def about_us_display():
    ...
    def this_list1():
        ...
        Lb1 = tk.Listbox(top)
        Lb1.insert(1,"You can receive from:")
        Lb1.insert(1, "O-")
    ...
    #start_info = tk.Label(A,text='We would like you to know which blood 
    # groups are realted.here is a table you can refer.')
    ...
    made_by = tk.Label(A, text='Project Made by:')
    n = tk.Label(A, text='Aditya Nandeshwar')
    k = tk.Label(A, text='Aniketh Mahadik')
    l = tk.Label(A, text='Atreay Kukanur')
    m = tk.Label(A, text='Fakkiresh Noorshetter')

    #start_info.grid(row=1, column=1)
    info_.grid(row=3, column=1)
    made_by.grid(row=22, column=1)
    k.grid(row=23, column=1)
    l.grid(row=24, column=1)
    n.grid(row=25, column=1)
    m.grid(row=26, column=1)
    A.mainloop()
